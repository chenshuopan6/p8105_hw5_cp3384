...
which means that as Week increases, Value also increases. \# Question 3
...
Comment: As can be seen from the figure above, for the test H:$\mu = 0$:
as the true average value used by our simulated data increases, the
proportion of rejecting the null hypothesis increases. When the real mu
value is 5 or greater, We will almost reject H:\$= 0 \$ for all the
...
because we set H:\$= 0 \$, and we will reject the hypothesis only when
the predicted value of a sample is significantly greater or less than 0.
